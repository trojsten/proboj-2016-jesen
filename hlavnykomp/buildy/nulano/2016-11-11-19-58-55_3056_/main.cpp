...
while (!q.empty()) {
 ...
 pole = gs.get_block(p.x,p.y);
    if (((p.x!=x||p.y!=y)&&pole.crossed_by == ja) || pole.type == WALL || pole.type == FAST_BONUS || x < 0 || y < 0) continue;
    if (m==0 && pole.owned_by != ja) {
break;
}
    if (m>0 && (pole.owned_by == ja || pole.type == STONE_BONUS)) {
break;
}
