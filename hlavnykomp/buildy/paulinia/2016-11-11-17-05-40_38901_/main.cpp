...
long double compute_score(long double PS, long double RU, long double A, long double D = 1){ //IMAGINE a MAGIC
    if(len <= max_safe_len){
        return pow(PS, - pud_sebazachovy) + pow(A, -attack_const) + pow(RU, -(rozsirovanie_uzemia));
    }
    else if(D - PS > defense_const * max_safe_dist){
        return pow(D, defense_const);
    }
    else{
...
int compute_direction(vector<int> moze, vector<int> PS, vector<int> RU, vector<int> A, vector<int> D){
    int minit = rand() % PS.size();
    FOR(i, PS.size()){
        cerr << "DIR " << S[moze[i]] << " A PS[i] = " << PS[i] << " a RU[i] = " <<  RU[i] << " score: " << compute_score(PS[i], RU[i], A[minit]) << endl;
        if(compute_score(PS[i], RU[i], A[i]) > compute_score(PS[minit], RU[minit], A[minit])) minit = i;
    }
    if(rand() % mutation_probab == 0) return moze[rand() % moze.size()];
    cerr << "minit = " << minit << " and score: " << compute_score(PS[minit], RU[minit], A[minit]);
    return moze[minit];
}
...
int find_direction(int x, int y){
    ...
    if(moze.size()){
        ...
        FOR(i, moze.size()){
            ...
            D.push_back(be_aware(x, y, moze[i]));
        }
