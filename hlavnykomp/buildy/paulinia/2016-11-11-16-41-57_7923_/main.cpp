...
long double compute_score(long double PS, long double RU, long double A, long double D = 1){ //IMAGINE a MAGIC
    if(len <= max_safe_len){
        return (pow(D, defense_const) + pow(PS, - pud_sebazachovy) + pow(A, -attack_const) + pow(RU, -(rozsirovanie_uzemia)));
    }
    else{
...
int compute_direction(vector<int> moze, vector<int> PS, vector<int> RU, vector<int> A){
...
int find_direction(int x, int y){
    ...
    if(moze.size()){
        ...
        int res = compute_direction(moze, PS, RU, A);
        return res;
    }
    return rand() % 4;
}
