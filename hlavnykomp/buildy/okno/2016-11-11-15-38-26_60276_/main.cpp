...
int main()
{
    ...
    while (true)
    {
        ...
        case 0:
            ...
            {
                if (min(vzdialenostBodu(vysiel, gs.players[i].position), vzdialenostBodu(gs.players[ja].position, gs.players[i].position)) <= cestaDomov - 1)
                {
                    smer = (smerVon - 1) % 4;
                    objective = CHODSPAT;
                    break;
        ...
        case 2: {
            ...
            for (int i = 0; i < gs.players.size(); i++)
            {
                if (gs.players[i].position.x > gs.players[ja].position.x) vacsiMensiX.first++;
                else vacsiMensiX.second++;
                if (gs.players[i].position.y > gs.players[ja].position.y) vacsiMensiY.first++;
                else vacsiMensiY.second++;
            }
            srand(time(NULL));
            vacsiMensiX.first += rand()%4;
            srand(time(NULL)+1);
            vacsiMensiX.second += rand()%4;
            srand(time(NULL)+2);
            vacsiMensiY.first += rand()%4;
            srand(time(NULL)+3);
            vacsiMensiY.second += rand()%4;
        ...
        case 3:
            if (gs.get_block(gs.players[ja].position.x + x_move, gs.players[ja].position.y).owned_by == ja) smer = (3 - x_move) % 4;
